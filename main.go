...
func run(ctx context.Context, tuning runtimeTuning) error {
	// Sudo requirement removed! Keyboard mashing works for users.
...
func listenForSlaps(ctx context.Context, pack *soundPack, tuning runtimeTuning) error {
	...
	fmt.Printf("\r\nspank: listening for MICROPHONE THUDS in %s mode with %s tuning... (ctrl+c to quit)\r\n", pack.name, presetLabel)
	if stdioMode {
		fmt.Print("{\"status\":\"ready\"}\r\n")
	}

	// Channel to receive amplitudes from our Swift sub-process
	amplitudes := make(chan float64, 10)
	micErr := make(chan string)

	go func() {
		// Verify mic_sensor binary exists
		if _, err := os.Stat("./mic_sensor"); os.IsNotExist(err) {
			micErr <- "ERR: ./mic_sensor binary not found. Please run 'swiftc mic_sensor.swift' first."
			return
		}

		cmd := exec.CommandContext(ctx, "./mic_sensor")
		...
		if err := cmd.Wait(); err != nil {
			// micErr <- err.Error() // Context cancel will cause an error, ignore
		}
	}()
	...
	for {
		...
		case <-ctx.Done():
			...
			return nil
		case errMsg := <-micErr:
			fmt.Printf("\r\n[Fatal Microphone Error] %s\r\n", errMsg)
			return fmt.Errorf("microphone listener failed: %s", errMsg)
		case amplitude := <-amplitudes:
			...
			// To filter out gentle talking, apply the min-amplitude flag limit
			if amplitude < tuning.minAmplitude {
				continue
			}
